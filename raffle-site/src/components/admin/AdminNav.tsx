...
function AdminNav({ onNavigate, currentSection }: AdminNavProps) {
  ...
  const handleEventChange = (eventId: string, section: string) => {
  ...
  const renderEventSelector = (section: string) => (
    <div className="pl-4">
      <select
        value={selectedEventId}
        onChange={(e) => handleEventChange(e.target.value, section)}
            ...
            <li key={item.id}>
              {item.id === "items" || item.id === "orders" ? (
                <div className="space-y-2">
                  ...
                  {currentSection === item.id && renderEventSelector(item.id)}
                </div>
              ) : (
                <button
                  onClick={() => onNavigate(item.id)}
