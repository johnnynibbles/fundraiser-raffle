...
CREATE
OR REPLACE FUNCTION public.handle_updated_at () RETURNS TRIGGER AS $$ BEGIN
    NEW .updated_at = NOW ();
...
CREATE TABLE IF NOT EXISTS public.user_profiles (
    id UUID REFERENCES auth.users (id) ON
    DELETE
        CASCADE PRIMARY KEY,
        email TEXT UNIQUE,
        full_name TEXT,
...
CREATE TABLE IF NOT EXISTS public.raffle_items (
    id UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    event_id UUID REFERENCES public.raffle_events (id) ON
    DELETE
        CASCADE,
...
CREATE POLICY "Allow authenticated users to delete their images" ON storage.objects FOR
DELETE
    USING (
        bucket_id = 'raffle-items'
        AND auth.role () = 'authenticated'
...
CREATE POLICY "Enable insert for authenticated users only" ON public.raffle_events FOR INSERT
WITH CHECK (
   EXISTS ( SELECT
        1
    FROM user_profiles
...
CREATE POLICY "Enable delete for authenticated users only" ON public.raffle_events FOR
DELETE
    USING (
        EXISTS (
            SELECT
...
CREATE POLICY "Enable insert for admin users only" ON public.raffle_items FOR INSERT
WITH CHECK (
...
CREATE POLICY "Enable delete for admin users only" ON public.raffle_items FOR
DELETE
    USING (
        EXISTS (
            SELECT
...
CREATE TABLE IF NOT EXISTS public.order_items (
    id UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    order_id UUID REFERENCES public.orders (id) ON
    DELETE
        CASCADE,
        item_id UUID REFERENCES public.raffle_items (id) ON
    DELETE
        CASCADE,
...
CREATE POLICY "Enable delete for admin users only" ON public.orders FOR
DELETE
    USING (
        EXISTS (
            SELECT
...
CREATE POLICY "Enable delete for admin users only" ON public.order_items FOR
DELETE
    USING (
        EXISTS (
            SELECT
